defmodule Day5.Part1 do
  ...
  def process_instruction(stacks, count, from, to) when count > 0 do
    crate = stacks[from] |> List.first()

    stacks =
      stacks
      |> Map.put(from, stacks[from] |> Enum.drop(1))
      |> Map.put(to, [crate | stacks[to]])

    process_instruction(stacks, count - 1, from, to)
  end
  ...
  @doc """
      iex> Day5.Part1.parse_stacks(["[Z] [M] [P]", "[N] [C]    ", "    [D]    "])
      %{1 => [?N, ?Z], 2 => [?D, ?C, ?M], 3 => [?P]}
  """
  ...
  @doc """
      iex> Day5.Part1.parse_instructions(["move 1 from 2 to 1", "move 3 from 1 to 3"])
      [{1,2,1}, {3,1,3}]
  """
